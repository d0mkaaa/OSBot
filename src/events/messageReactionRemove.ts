...
const event: BotEvent = {
  ...
  async execute(reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser): Promise<void> {
    ...
    await starboardManager.handleReaction(reaction, user, false);
    ...
    try {
      ...
      logger.info(`Removed role ${role.name} from ${user.tag} via reaction role`);
    } catch (error) {
      logger.error('Failed to remove role via reaction', error);
    }
  }
};
